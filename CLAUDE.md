...
- Print uses `window.print()` + `@media print` rules in `src/styles.css`. Anything that shouldn't print (controls, saved designs, buttons, the buy-list inputs) is marked `className="no-print"`; the preview and cut-list cards are `className="card"`, and the scrollable cut-list table is `className="cut-table"` so print can un-clip it.
- **Keep `Field`, `Section`, and `Panel` at module scope.** They were moved out of `WallPanelingPlanner` because defining a component inside the render recreates its type every render, which remounts inputs and steals focus mid-typing. `Field` takes `unit` as a prop (callers pass `unit={unit}`); `Panel` takes its geometry/handlers as props.
